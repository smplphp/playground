 ...
 * Because of this, it is not safe to rely on an elements' index, as that index
 * is subject to change.
 *
 * Sequences are always indexed in a range of [0, n), where n is the number of
 * elements within the sequence.
 * This means that when appending a new element to the end of the sequence, its
 * index will be n-1.
 *
 ...
 *
 * @extends \Smpl\Collections\Contracts\Collection<ElType>
 */
interface Sequence extends Collection
{
     ...
     *
     * This method removes an element from the collection by its index.
     *
    ...
    /**
     * Get an element from the collection by its index
     *
     * This method returns an element at a given index within the collection.
     *
     * Implementations should ensure that this method only accepts values within
     * the range of [0, n).
     *
     ...
     *
     * This method returns the current index for a given element, if found, or
     * false otherwise.
     * This method also accepts an optional {@see \Smpl\Logic\Contracts\Comparator}
     * to determine whether an element matches.
     *
     * This method will only return the index for the first matching element.
     *
     ...
     *
     * This method inserts a given element at a given index if the index is
     ...
     * If the provided index is within the range of [0, n), the element currently
    ...
    /**
     * Insert multiple elements start at an index
     *
     * This method inserts multiple elements into the collection starting at a
     ...
     * x is its order in the provided elements.
     * For example, the first element will be inserted at i+0, then the next i+1,
     * and so on and so forth.
     *
     * Indexes for insertion should be within the range of [0, n].
     * If the provided index is within the range of [0, n), all elements that exist
     * at indexes that would be overlapped, will be shifted right 1 for each element
     * that is inserted.
     * If the provided index is n, then this method should function identically
     * to {@see self::addAll()}.
     *
    ...
    /**
     * Sets the element for an index
     *
     * This method sets the element at a given index, if the index is valid,
     * returning true of the collection was modified, and false otherwise.
     * Indexes should be within the range of [0, n), and if an element
     ...
     *
     * This method sorts all elements within the collection using the provided
     * {@see \Smpl\Logic\Contracts\Comparator}.
     * While it's possible that the order of the elements within the collection
     * remains the same, it should be assumed that all elements will have
     * different indexes after this operation.
     *
     * @param \Smpl\Logic\Contracts\Comparator<ElType> $comparator
     *
     * @return static
     */
     ...
     *
     * This method creates a new collection containing only elements within
     * the provided range.
     *
     ...
     * negative, it will start that many elements from the end of the collection.
     *
     * If count is positive, and within the range of (0, n], that many elements
     * will be contained within the resulting collection.
     * If count is negative, and within the range of [-n, 0), the new collection
     * will contain all elements from the given index, to this number of elements
     * from the end.
     *
     * @param int      $index
